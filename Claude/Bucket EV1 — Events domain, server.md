...
- **`venues`** — `name`, `description`, `address`, `city`, `state`, `zip`, `url`, `lat`/`lng` (nullable — see OQ2), `neighborhood_id` (nullable FK → neighborhoods), `status` (string, CHECK in GD1 §5 enum, default `'open'`), `last_verified_at_us` (nullable BIGINT), `created_us`, `updated_us`.
...
2. **`venues.lat/lng` column type:** use the honuware double/real column type if `DatabaseInfo` offers one; otherwise store as nullable strings until a numeric type exists (nothing consumes lat/lng at MVP — no map view yet). *Rec: check `database_info.h` at implementation time; don't block on it; strings are an acceptable stopgap for a write-only field.*
3. **Datetime entry in the generic admin editor:** `admin_column_data_info`'s input types are text/number/bool/date — if the date control turns out date-only (no time-of-day) for `starts_at_us`, hand-entering event times via generic CRUD is clunky. *Rec: verify during §2.6; if clunky, don't extend honuware now — the `ingest-demo`/`create-event` test_helper commands cover dev, EV3's submit form becomes the real human intake, and EV2 may add a small `manage_events`-gated "quick add" form if the manual loop needs it before EV3.*
4. **"Minor vs major" ingest fields:** title is classed **minor** (silent update) — only date/venue changes revert approval, per SUTP Q4c's wording. *Rec: as pinned; a retitle that survives review isn't worth re-review friction.*
5. **Series URL namespace `/api/event_series`** (not `/api/events/series`) to avoid colliding with `/api/events/<int>`. Client-facing page URLs (EV2) stay `/events/series/:slug` — only the API path differs. *Rec: as pinned.*
6. **`categories` on series:** series carry no direct category/scene assignments at MVP — their instances carry them, and the series page derives chips from its instances. *Rec: as pinned; add `series_scene_tag_assignments` later only if a series page needs tags with zero upcoming instances.*

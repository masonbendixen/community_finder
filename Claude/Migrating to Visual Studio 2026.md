...
- [ ] Belt-and-braces, per machine: Tools → Options → CMake → *"Prefer using CMake Presets…"* → **Never**. Still worth setting — it makes the mode explicit rather than dependent on which files exist — but it should now be understood as **hygiene, not the fix**.
- [ ] Merge the repo-root launch configuration's `env` and `args` into whichever workspace is actually used, so that debugging `database_helper` runs with `HONUWARE_ALLOW_DESTRUCTIVE=1` and `--recreate_database` rather than bare. This is what unblocks 16.1.
...
**Where to put it.** In Visual Studio, add to the `"args"` array in `launch.vs.json` — the same place the existing `--recreate_database` entry sits. In the container, arguments pass straight through: `./docker/build_and_test.sh --gtest_filter=Event*`.
...
**Correction to the original framing of this item: the README already exists.** `server_components/README.md` is 8271 bytes with sections *The layer stack*, *What is NOT here*, *Build & test*, *Consuming these components*, *New-consumer checklist* and *License* — and it already documents the `HONUWARE_DB_*` variables and the legacy `KNOTTYYOGA_DB_*` fallback at line 92. So this is an **extension of an existing document**, not a new one, and the work is to add the machine-setup material that is missing rather than to restate what is there. Writing a second setup document beside it would be the worst outcome.
...
- [ ] **Include the Visual Studio CMake-mode setting as a numbered setup step**, not as troubleshooting:
  ...
  > Tools → Options → CMake → *"Prefer using CMake Presets…"* → **Never**

  The reason belongs with it: these repos are driven by `CMakeSettings.json`, which is where the `CONAN_CMD` and `CMAKE_PROJECT_TOP_LEVEL_INCLUDES` wiring lives, and VS silently switches to Presets mode whenever a `CMakeUserPresets.json` happens to be present. **Setting it to Never makes the choice explicit rather than dependent on which files happen to exist in the tree at the moment VS opens** — which matters here because the Linux gate writes that file into the Windows working tree on every run (8.2). Even once 8.2 suppresses the file, keep this step: it makes a new machine deterministic instead of relying on the suppression having been applied everywhere.
- [ ] Carry over the environment-variable table from **8.3** — it is the reference a new developer needs and it exists nowhere today.

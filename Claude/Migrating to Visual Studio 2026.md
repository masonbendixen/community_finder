...
- [x] **Revoke the existing app password** at the Google account's App Passwords page. Do this first and independently of any code change — the credential is already disclosed and every hour it stays valid is exposure. ✅ 2026-09-10 — revoked and a replacement minted. **One revocation confirmed, and there were two credentials** (see the correction below): which of the two was revoked has not been established, so the "verify BOTH" item in 9.4 stays open. The instruction to revoke before touching code was followed — the code changes in 9.2 landed afterwards.
...
- [x] **Correction 2026-09-10: these are TWO DIFFERENT app passwords, not one.** This item previously said the launch profile carried "the same app password". It does not. Verified by comparing the literals:
...
- [x] **The local-disk inventory has grown since this item was written, partly as a side effect of 8.1's tooling.** *(Resolved 2026-09-10: the revoked value is now gone from all three locations — the scratchpad backup was deleted, and `launch_defaults.local.json` plus its generated `.vs/launch.vs.json` carry the new credential. A full recursive scan of all three repos and the scratchpad returns nothing.)* Current known copies of `rquya…kwuyc`, targeted search (not exhaustive):
...
- [x] Worth noting what this copy also reveals: communityfinder already reads the password from an **environment variable** (`HONUWARE_MAIL_APP_PASSWORD` appears in `app_secret_values.cpp` and `create_database.cpp`), so the app side of 9.2 may largely exist already and the literal in honuware's `secret_values.cpp` is the outlier. Confirm before designing a new seam — the mechanism is probably already there. ✅ 2026-09-10 — **confirmed for communityfinder, and the confirmation found the gap that mattered: knottyyoga had NO app-side seeding at all.** `kMailAppPassword` / `mail_app_password` appeared nowhere in that entire repo; it was silently inheriting the framework default. Emptying that default in 9.2 would have left knottyyoga unable to send mail, so it gained the same env-var seeding (see 9.2). *"The mechanism is probably already there" was true of one app and false of the other — checking both is what caught it.*
- [x] Issue a replacement and put it **only** in the `config_secrets` / environment path the rest of the system already uses. Do not commit it. ✅ 2026-09-10 — new credential minted and placed in `communityfinder_server/launch_defaults.local.json` (gitignored), scoped to `*database_helper.exe*`, from which `.vs/launch.vs.json` is generated. It reaches `config_secrets` only via the seeder's UPDATE. Not committed anywhere.
- [x] **Understand what rotation does and does not fix.** Removing the value from `HEAD` does **not** remove it from git history — it stays in every clone, every fork, and GitHub's raw object store. Rotation is what actually closes it. ✅ 2026-09-10 — and confirmed concretely: after removing the literal from `secret_values.cpp`, `git log -S` still finds it in **1 commit** (`f95d099`, the initial extraction). The working tree is clean; the history is not, and never will be without a rewrite.
- [x] **Decided (New OQ 18): rotate, do not scrub history.** Once the password is revoked the value in history is inert, and a `filter-repo`/BFG rewrite would rename every commit SHA — invalidating the honuware SHA both apps pin (`6abea141…` today), breaking every existing clone, and landing that cost directly on the work in this plan. **The one thing that must not slip: the rotation is what closes this, so it cannot be deferred behind the code change in 9.2.** Revoke first, tidy the code afterwards.
...
- [x] Replace that literal with an **empty** default, reusing communityfinder's own reasoning verbatim — an unconfigured build must fail to send rather than authenticate as the shared account. ✅ 2026-09-10 — `secret_values.cpp:18` is now `""`, with the reasoning in a comment above it so the next reader meets the rule at the point of temptation.
- [x] **Understand why it happened, or it will happen again.** Per `secrets/CLAUDE.md`, defaults in `secret_values.cpp` are loaded *"into the database on first run (production)"* **and** *"into the test secrets helper automatically (tests)"*. Putting the password there made every test everywhere work with zero configuration. **That convenience is the trap**: the defaults file is the one place a secret must never go, and it is also the most convenient place to put one. ✅ 2026-09-10 — written into `secrets/CLAUDE.md` as the stated reason, not just the rule.
- [x] Add the rule to `components/services/util/secrets/CLAUDE.md`, which currently explains *how* to add a secret's default value without ever saying that a real credential must not have one. The document that taught the pattern is the document that should forbid this. ✅ 2026-09-10 — new lead section *"A real credential NEVER gets a compiled-in default"*, ahead of *Adding a New Secret*, carrying the three-tier table, the public-repo/history consequence, the if-it-can-sit-in-a-public-repo-it-is-not-a-secret test, and the requirement that an empty default fail loud.
- [x] **Test:** a missing mail secret produces a clean, diagnosable failure rather than an attempt to authenticate with an empty password. `server_config.cpp:173-194` already has the "fail loud on missing required secret" shape to follow. ✅ 2026-09-10 — `MakeMailHelper(Transaction&, SecretsHelperPtr)` now throws `std::runtime_error` naming both `config_secrets.mail_app_password` and `HONUWARE_MAIL_APP_PASSWORD`. Three tests: `FrameworkShipsNoMailPasswordDefault` (the regression guard that fires if anyone re-adds a literal), `MakeMailHelperFailsLoudWhenPasswordMissing`, `MakeMailHelperSucceedsWhenPasswordSeeded`.

- [x] **Unplanned but required: give knottyyoga the same seeding.** ✅ 2026-09-10 — this phase was scoped from communityfinder's implementation and assumed the app side was already solved everywhere. It was not: knottyyoga referenced `kMailAppPassword` **nowhere**, relying entirely on the framework default. Emptying that default would have silently left it unable to send mail — the failure would have been diagnosable thanks to the new fail-loud, but there would have been no route to a working password at all. `create_database.cpp` → `PopulateConfigSecrets` now mirrors communityfinder's UPDATE, with `kEnvMailAppPassword` and a `util/env.h` include. Verified: **5168 tests from 548 suites, all passed.**

  **The two apps deliberately share one credential for now.** Both seed `knottyyogaandspa@gmail.com` as `kMailSenderAddress`, so both read the same `HONUWARE_MAIL_APP_PASSWORD`. Accepted as temporary so both can send mail during development; knottyyoga gets its own at deploy time, tracked in *Knotty Yoga → Deploying to AWS → "AT DEPLOY: rotate the Gmail app password and give knottyyoga its own"*. **CommunityFinder will need the same treatment** — it keeps the shared mailbox otherwise.
...
- [x] Drive it through the existing `TestMailHelper` double, leaving genuine sending to the `--send_real_email` path the test helper already has. ✅ 2026-09-10 — `MailHelperTest.SendMessage` is replaced by `SendMessageReachesTransportWithComposedContent`. `SendMessageInvalidMethod` was left alone deliberately: it also calls `MakeMailHelper`, but `ConvertAuthMethod` throws before any socket is opened, so it never sent mail.
- [x] **Test:** the double-based test must still assert what the live one did — that the composed message reaches the transport with the right recipient, subject and CRLF-correct body. The existing CRLF coverage (`quick_account_welcome_mail_test.cpp`, `person_verify_mail_test.cpp`, `types_test.cpp`) stays as-is; this is about the send path, not the formatting. ✅ 2026-09-10 — asserts sender, recipient, subject and text body, then walks the HTML body asserting **every** `\n` is preceded by `\r`. Asserting the property rather than a fixed string, so it keeps holding if the body content changes. Note the old test asserted strictly less than this: `EXPECT_NO_THROW` on a live send proved the SMTP server accepted it, never what was composed.
- [x] Expect the suite to get **~1.7 s faster** and to stop depending on network egress and a third party's availability — worth noting because it also removes a flake source from CI. ✅ 2026-09-10 — the whole `MailHelperTest` suite is now **14 tests in 87 ms**, where one test alone previously took ~1681 ms of live SMTP. Full suite green: **1767 tests from 177 suites, 112.6 s, all passed.**

**A caveat on that green run, in the spirit of the rest of this document.** 1767 passing tests confirm nothing about *sending* mail any more — that path now has no automated coverage at all, by design. The verification that the rotated credential actually authenticates is a **manual** step: seed a database with the new `HONUWARE_MAIL_APP_PASSWORD` and use the test helper's `--send_real_email`. Until that is done, "green" means the composition is right, not that mail works.
...
- [x] **Rule to write into `secrets/CLAUDE.md` alongside 9.2:** if a value can sit in a public repo it is not a secret (sender address, SMTP host, port, brand strings all belong in `*_secret_values.cpp`); a secret never gets a compiled-in default; and exactly one bootstrap secret (`HONUWARE_SECRET_KEY`) lives in the environment with everything else behind it. ✅ 2026-09-10 — all three, plus the fail-loud requirement for empty defaults.

**Status of the rotation itself, 2026-09-10.** The new credential was minted and placed in the sanctioned location: `communityfinder_server/launch_defaults.local.json`, scoped to `*database_helper.exe*`, from which `.vs/launch.vs.json` is generated. The revoked value is **gone from disk** — verified by a full recursive scan of all three repos and the working scratchpad, which now returns nothing; the scratchpad backup of the old root workspace that held a copy was deleted.

- [ ] **The master copy is still a plaintext file: `C:\Users\mason\Documents\GMailAppPassword.txt`.** This is the exact pattern this section argues against. One mitigation applies — measured above, `C:\Users\mason\Documents` is the **non-synced** folder, so it is not going to OneDrive — but that is a distinction two folders deep, and it is the reasoning this section already warned against relying on. Move it into a password manager, or delete it and rely on the launch profile plus the seeded `config_secrets` row, which is what the tiering is for. `npm_recovery_codes.txt` sits in the same folder and outranks it.
- [ ] **Verify BOTH old credentials are revoked, not one.** Per the correction in 9.1 there were two distinct app passwords in play, and only one revocation has been confirmed. The one that was *committed and public* (`ctojsn…lbdz`, in `secret_values.cpp` since the initial-extraction commit) is the one that matters most, and removing it from `HEAD` today does **not** revoke it — it remains valid until revoked at Google, and remains in git history regardless.
...
- [x] Note the seam that makes this cheap: `global_database_test_support.cpp:21` holds `static std::string name{kTestDatabaseName}`, set by `Initialize`, so the name is **already a runtime value with an app-supplied override**. communityfinder overrides it; knottyyoga does not. ✅ 2026-09-11 — confirmed, but the suffix is applied **one step earlier** than this seam suggested: at the app boundary, before `MakeDatabaseInfo`. `DatabaseInfo` has no name mutator, so suffixing inside `Initialize` would have left `GetDatabaseInfo().GetDatabaseName()` naming a database that does not exist — and two callers read it back (`endpoint_test_helper.cpp:78`, `database_metadata_test.cpp:54`). Composing at the boundary keeps the schema's name and the physical name the same string by construction.
- [x] **A wart to fix while here:** `global_database_test_support.h:12` reads `constexpr std::string_view kTestDatabaseName = "test_knottyyoga"`. **The framework's default is one specific application's database name.** knottyyoga works only because it inherits it. ✅ 2026-09-11 — now `"honuware_test"`, honuware's own, with knottyyoga naming itself in the same change (10.2b).
...
- [x] Derive the suffix at **compile time** from the platform — `_windows` under `_WIN32`, `_linux` otherwise — appended to the app-supplied base name. ✅ 2026-09-11 — `kTestDatabasePlatformToken` + `ComposeTestDatabaseName(base, token)` in `global_database_test_support.h`; each of the three test mains composes its own base name. Six databases from three base names:
...
- [x] **Why compile-time beats the environment variable, now that two slots is the answer.** *(Reasoning recorded verbatim in the header comment, so the next reader meets it at the code rather than only here.)* The harness **DROPs and CREATEs** whatever name it is handed. An env-supplied suffix therefore makes an arbitrary, externally-controlled string part of a destructive database operation, which would have needed validation, a character allowlist and a length check against PostgreSQL's 63-byte identifier cap — all to support a third slot nobody asked for. Compile-time detection cannot be misconfigured, needs no validation, and needs no wiring in any harness. **Fewer moving parts and strictly less risk.**
- [x] Accept the trade explicitly: with no env override, two Linux gates cannot run concurrently (two checkouts of the same repo would collide). That is precisely what OQ16 said is fine. If it ever stops being fine, the escape hatch is an override — and it should then arrive *with* the validation described above, not without it. ✅ 2026-09-11 — stated in the header comment and in all four consuming docs, so the limit is discoverable from the code and from the gate scripts rather than only from this plan.
- [x] **Tests:** the composed name carries the right suffix for the built platform, and the base name is preserved. Since the branch is `#ifdef`, one side is unreachable per build — so assert the composition function's behaviour against an explicitly-passed platform token rather than only the compiled default, or the test proves nothing on the other platform. ✅ 2026-09-11 — five tests in `global_database_test_support_test.cpp`, all passing: both tokens asserted explicitly (including all three real base names), base-name preservation, the empty-token case (no stray trailing underscore — the shape an override would take), the compiled token agreeing with the build platform, and an end-to-end check that asks PostgreSQL `SELECT current_database()` rather than trusting a constant.

  **A trap in that last test, caught before it bit.** The first version asserted `current_database() == ComposeTestDatabaseName(kTestDatabaseName)`. That file compiles into *every* app's suite, and each app supplies its own base name — so it would have failed in knottyyoga and communityfinder for entirely the wrong reason. The framework-level invariant is "whatever the base name, the physical database is platform-qualified **and** matches the injected schema", which is what it now asserts. *A test written from the framework's own perspective was wrong precisely because the framework is not the only consumer — the same mistake, in miniature, as the `kTestDatabaseName` default this phase is removing.*
...
- [x] Change `global_database_test_support.h:12` from `kTestDatabaseName = "test_knottyyoga"` to honuware's own `"honuware_test"`. The framework should not default to one application's database. ✅ 2026-09-11
- [x] Give knottyyoga's test main an explicit base name, mirroring `communityfinder/server/communityfinder_server/test/src/main.cpp:24`. After this all three are symmetric: each app names its own database, and honuware's default is honuware's own. ✅ 2026-09-11 — `kAppTestDatabaseName = "test_knottyyoga"` in knottyyoga's test main, composed through `ComposeTestDatabaseName`.
- [x] **These two must land together.** If the honuware default changes and knottyyoga has not yet set its name, knottyyoga's suite silently starts building its schema in `honuware_test` — wrong database, still green. Push honuware → re-pin → knottyyoga in one bump point, and **verify by checking which database the run actually touched**, not merely that it passed. ✅ 2026-09-11 — **verified by observation, not inference.** The end-to-end test now calls `RecordProperty("active_database", ...)`, so a run *reports* its database into `--gtest_output=xml` instead of only asserting a property of it. Read back from both suites:

  | suite | database actually used |
  |---|---|
  | `honuware_test_runner` | `honuware_test_windows` |
  | `knottyyoga_tests` | `test_knottyyoga_windows` |

  knottyyoga is on its own database, not `honuware_test` — which is exactly the silent-wrong-database failure this item exists to catch. *The reason this needed observing rather than reasoning: the failure mode is a green suite. Nothing about the result would have looked wrong.*
...
- [x] Update the three READMEs that document the database-per-suite arrangement (`communityfinder/database_server/README.md:42-50`, `knottyyoga/server/docker_project/README.md:62-69`, `communityfinder/server/docker/README.md:65`), plus the `build_and_test.sh` header comments in both apps, which name the databases explicitly. ✅ 2026-09-11 — and **three** `build_and_test.sh` headers, not two: honuware's own carried the same warning at `docker/build_and_test.sh:106-108`.

  **All of them carried an instruction that is now exactly backwards** — *"never run the SAME suite from Windows and Linux at once"*. That was the true and load-bearing warning this phase exists to retire, so each one now says the opposite explicitly, rather than being quietly edited: the collision is gone, and here is the limit that remains.
- [x] `communityfinder/CLAUDE.md:108` names `test_communityfinder`; update it too. ✅ 2026-09-11
- [x] Mention that the **old unsuffixed databases become orphaned** (`honuware_test`, `test_knottyyoga`, `test_communityfinder`). They are dropped-and-recreated scratch databases with nothing to preserve, so they can simply be dropped by hand once — worth a line in the README so nobody wonders what they are. ✅ 2026-09-11 — in all four documents. Also added the `max_connections` warning from 10.4 to communityfinder's docker README, since that is where someone running several gates at once will be looking.
...
- [x] communityfinder `1000` → **1700**. ✅ 2026-09-12 — actual **1793** (the item said 1785; Phases 10–11 moved it since). Validated live: the gate printed `tests executed: 1793 (floor: 1700)`.
- [x] knottyyoga `3500` → **4800**. ✅ 2026-09-12 — actual **5173**. Validated live: `tests executed: 5173 (floor: 4800)`.
- [x] honuware CI `MIN_EXPECTED_TESTS: 1000` → **1700**. ✅ 2026-09-12 — actual **1772**. Raised in BOTH places: `ci.yml` and `docker/build_and_test.sh`, which this item treats as one but which are two independent floors.
- [x] ✅ 2026-09-12 — all four carry the rule, the measured count and the date. **The CI comment being replaced was itself the proof:** it read *"~1308 TEST()s exist across 114 component files today"* — written once, never revisited, wrong by 464 tests. A floor of 1000 against 1772 could no longer detect 43% of the suite vanishing, which is most of what it was for. Original: Add a line to each script's comment saying the floor tracks the count and must be raised as suites grow — the shared root cause is that all three were set once and never revisited.
- [x] **Record the count in this document on every gate run from now on.** Phase 7.3 proved why: knottyyoga's written-down 5163 made `+2 = the two TIFF tests` a precise claim, while communityfinder's missing baseline left only "it went up".
...
- [x] ✅ 2026-09-12 — done exactly so; SHA-256 `d6c83076…c6cc` was read from Kitware's own published checksum file inside a container rather than transcribed by hand. Original: **Decided (New OQ 17, Mason deferred to me): the official binary tarball, pinned by version and SHA-256.** Reasoning, since the choice was left open: it adds no third-party package source to the images, it installs the *exact* build the dev boxes run rather than whatever a repository currently serves, and the checksum makes the image reproducible in a way `apt-get install cmake` never was. Kitware's APT repo is the conventional choice but reintroduces "whatever is current"; `pip install cmake` drags a Python dependency into images that otherwise need none — and given 6.2b, adding an incidental Python requirement to a build image is precisely the kind of thing that bites later.
- [x] ✅ 2026-09-12 — pinned 4.4.3. **But "matching the dev boxes" is only half true.** This machine has TWO CMakes: `C:\Program Files\CMake` at **4.4.3** (on PATH) and Visual Studio 2026's bundled **4.3.1-msvc1**, which is what VS actually builds with. Linux and CI now match the standalone install, not what the IDE runs. The gap that mattered (3.25 → 4.x) is closed; the platforms are still not identical. Pointing VS at the standalone install via Tools → Options → CMake would close the rest. Original: Pin the **exact** version — 4.4.3, matching the dev boxes. "Latest" reintroduces the drift this is meant to remove.
- [x] ✅ 2026-09-12 — all five moved together. **Decided: all five images move together, including the release image.** New OQ 17 asked this explicitly and Mason left it to me, so recording it plainly rather than letting it pass silently: `knottyyoga/server/knottyyoga_server/package/Dockerfile` builds **what actually ships**, and this change alters the toolchain that produces the shipped artifact. The OQ6 argument is what settles it — raising CI without raising the deploy image trades one divergence for a strictly worse one, because CI would then verify a build nobody performs. If that is not wanted, this is the bullet to object to.
- [x] ✅ 2026-09-12 — verified no `apt` cmake remains on any surface, and both rebuilt app images report `cmake version 4.4.3`. Original: Apply to all five in one change. A partial raise is worse than none: it would mean three different CMake versions across the gates.
- [x] ✅ 2026-09-12 — **it flipped and nothing broke.** Boost resolved through `find_package(Boost)` in config mode on Linux, all three gates green. The risk was real and correctly identified; it simply did not materialise. Original: Expect `if(POLICY CMP0167)` to become **true** on Linux for the first time, flipping the Linux Boost lookup from module mode to config mode. That is the entire point, and it is also the one thing that could break — Phase 1.2's config-mode verification was done against Windows-generated Conan files only.
- [x] **Gate:** all three Linux suites green at their new floors. ✅ 2026-09-12 — honuware **1772** (floor 1700), communityfinder **1793** (floor 1700), knottyyoga **5173** (floor 4800). Each gate line shows the raised floor, so 12.1 and 12.2 are jointly verified by the same runs.

  **A tagging slip worth recording.** The pattern was first proved in an image tagged `honuware_build:cmake443-test` — deliberately, to avoid overwriting a working image before the change was known good — and then that throwaway tag kept being used. `honuware_build:latest` still held CMake 3.25 while every reported result came from the test tag. Rebuilt under the real tag, test tag deleted. *A scratch tag used past the point it is scratch is indistinguishable from the real thing in every log line it produces.*
...
- [x] `conan lock create` per repo, committed beside each `conanfile.py`. ✅ 2026-09-12 — `server_components/conan.lock` (2768 B) and 2910 B each for the two apps.
- [x] Decide whether Linux and Windows share one lockfile or take one each. ✅ 2026-09-12 — **ONE lockfile per repo, shared.** Answered by measurement rather than preference: a Linux lockfile was generated first, then extended with the Windows configuration, and every package resolved to a single version — `libpq/17.7`, `xz_utils/5.8.3`, `zstd/1.5.7`, `boost/1.86.0`, `openssl/3.5.8`. **6.2b's premise no longer holds:** the platforms diverged then and do not now, because the Phase 4/5 recipe bumps and the CMake raise converged the graph on their own. A per-platform lockfile would have been two copies of one answer.
- [x] ✅ 2026-09-12 — wired into **six** surfaces, not the four this item implies: three Linux gate scripts, CI, and the three Windows presets. Windows runs Conan through `conan_provider.cmake`, which forwards a `CONAN_INSTALL_ARGS` cache variable, so the presets carry `--build=missing;--lockfile=${sourceDir}/conan.lock`.

  **Confirmed load-bearing rather than assumed.** Both platforms log `Using lockfile:`, and Windows *rebuilt libpq 17.7 from source via Meson* instead of silently reusing a different cached binary. That rebuild IS the lockfile working — a decorative one would have been free. Original: Wire the gates and CI to `--lockfile`, otherwise the file is decorative — the same trap as the committed Conan profile in Phase 2 that did nothing until it was named in `CONAN_HOST_PROFILE`.
- [x] **Converge upward if they must converge**, per 6.2b. ✅ 2026-09-12 — they already had. The locked `libpq/17.7` sits on the Meson side of that split, and the Windows source build through Meson succeeded — the toolchain-agnostic behaviour 6.2b predicted. **Two cross-checks fell out for free:** `libtiff` is absent from all three lockfiles, confirming Phase 11 at dependency-graph level; and `libpq/17.7` is exactly what the three passing Linux gates had already used, so the lock captures a known-good set rather than a fresh resolution nobody has run.

**Verified on both platforms, all three repos (2026-09-12).** Every Windows run below is under its lockfile (`Using lockfile:` in each configure); every Linux result is on CMake 4.4.3, and honuware's Linux gate was re-run after the `--lockfile` wiring landed (`Using lockfile: '/src/conan.lock'`).
...
**Progress 2026-09-09: 14199 bytes. Completed 2026-09-12: 24331 bytes.** *Build & test* carries **Environment variables** (the 8.3 table) and **Debug targets in Visual Studio** (`tools/sync_launch_targets.ps1`); **Developer machine setup** now sits ahead of it, and `database_server/README.md` documents the shared PostgreSQL container that 14.2 moved here.

- [x] Add a **"Developer machine setup"** section ahead of *Build & test*, drawing on `knottyyoga/server/SERVER.md` and the vault's `Projects/Machine configuration.md`. ✅ 2026-09-12 — inserted at README line 44, ahead of *Build & test*. README is now **24331 bytes** (from 14199). Nine numbered steps plus a *Known setup failures* subsection. Written by **verifying each claim against the tree rather than transcribing `SERVER.md`**, which turned out to be stale: it documented the container as publishing `"5400:5432"` when `docker-compose.yml` and the running container both say **5432:5432**. Transcribing would have shipped that wrong port into the one document a new machine is supposed to trust.
- [x] Cover, in order: prerequisites (Git, CMake 4.4.3, Conan 2.31.2, Python, Visual Studio 2026) → clone → Windows/VS build → Docker/WSL setup → PostgreSQL → create and seed the dev database → running `honuware_test_runner` and the Linux gate. ✅ 2026-09-12 — all nine in that order: 1 Prerequisites, 2 Clone, 3 Configure Visual Studio, 4 First configure and build, 5 Generate the debug launch configuration, 6 Docker / the network / PostgreSQL, 7 Create and seed the dev database, 8 Know which database you are looking at, 9 Run the tests (both platforms, with what each catches that the other cannot).
- [x] **Include the Visual Studio CMake-mode setting as a numbered setup step**, not as troubleshooting: ✅ 2026-09-12 — step 3, carrying the **post-reversal** value (*use CMake Presets if available*) and the reasoning for making the mode explicit rather than emergent.
...
- [x] **Add generating the launch configuration as a setup step**, because the VS command that would normally do it is broken (8.1) and a new machine has no `.vs` at all: after the first successful configure, run `tools\sync_launch_targets.ps1 -RepoPath .`, then copy `tools/launch_defaults.example.json` to `launch_defaults.local.json` and fill in credentials. Without this, every debug target launches bare — no DB env, no `--recreate_database`, no `HONUWARE_ALLOW_DESTRUCTIVE`. ✅ 2026-09-12 — step 5, including the `launch_defaults.local.json` copy, why it is gitignored, and what breaks without it.
- [x] **State which folder to open in Visual Studio**, since both apps briefly had two candidate workspaces: open `server\<app>_server`, *not* the repo root (8.1). The Angular front end is opened separately. ✅ 2026-09-12 — step 3: repo root here, `server\<app>_server` in the apps, Angular opened separately, and the two-`.vs`-folders failure mode spelled out.
- [x] **Fold in the three machine-level failures this migration actually hit**, because each cost real time, each is a setup problem a new developer will hit identically, and none is discoverable from its error message: the Store-alias `python.exe` stub breaking Meson for libpq (6.2b), Git for Windows missing after a VS reinstall (7.1), and the **unseeded dev database** (16.1). ✅ 2026-09-12 — the *Known setup failures* subsection: the Store-alias `python.exe` stub, Git missing after a VS reinstall, and the unseeded dev database. Each says why its error message points nowhere near its cause. **A fourth was added from this phase's own work:** building with `cmake --build` from a plain shell rather than a developer prompt fails compiling every file with `Cannot open include file: 'algorithm'` — Ninja invokes `cl.exe` directly and `cl.exe` finds the standard library through `INCLUDE`, which only a developer prompt sets. It names a standard header, so it reads as a broken toolchain rather than a missing environment, and Visual Studio never reproduces it because the IDE supplies the environment.
- [x] **"Create and seed the dev database" must be an explicit numbered step**, with the `HONUWARE_ALLOW_DESTRUCTIVE=1` + `--recreate_database` invocation written out. It is currently documented nowhere — `Machine configuration.md` stops at cloning the repo — which is exactly why it was missed on a freshly-built machine. ✅ 2026-09-12 — step 7, with the command line written out, `HONUWARE_ALLOW_DESTRUCTIVE=1` and the exact-`"1"` requirement, both seed-time password variables and what each one's absence does (communityfinder leaves the row empty; knottyyoga throws), and the note that the F5 route only works once step 5 has been done.
...
- [x] Make the **two-database distinction** explicit, because a green suite hides this: the test suite drives `test_<app>` while the helpers and the server drive `<app>`. Creating one does not create the other, and 1785 passing tests say nothing about the dev database. ✅ 2026-09-12 — step 8, as a table, ending on the point that a green suite is entirely compatible with step 7 never having been run.
...
- [x] Move the database/container setup into `server_components`, leaving pointers behind in both apps rather than duplicating. ✅ 2026-09-12 — `server_components/database_server/` now owns it: `docker-compose.yml`, `postgres.conf`, `create_network.cmd`, `load_container.cmd`, `load_container_interactive.cmd`, `postgres_shell.cmd`, `remove_container.cmd`, `.gitignore` and a README. Pointer READMEs left in `knottyyoga/database_server/` and `communityfinder/database_server/`; no file is duplicated. **The 286 MB cluster moved with it** — container cleanly shut down first (`database system is shut down`), directory renamed on the same volume, restarted at the new path with `Skipping initialization` and no WAL replay. All 17 databases intact; knottyyoga still has its 118 tables, communityfinder 36.

  Three deliberate changes rather than a verbatim copy. **The `schema.sql` mount is gone** — it was a 2023 bootstrap that only ever ran against an empty data directory, creating a `knottyyoga` database and a six-row `classes` table that `--recreate_database` then rebuilds anyway; knottyyoga's own `CLAUDE.md` had called it unused for some time, and a shared container must not carry one app's tables. **The data path is a variable** (`HONUWARE_POSTGRES_DATA`, defaulting to `./data`) so an existing machine can point at a cluster rather than move it. **`postgres_shell.cmd` now uses `docker exec`** instead of `docker compose run` — the old form started a second container against the same data directory every time it was called, and the move surfaced **12 orphaned `database_server-postgresql-run-*` containers** dating back 18 months, since removed.
- [x] **Sequence this after Phase 10**, which changes the database-per-suite arrangement the current documents describe. Moving the text first would mean moving it twice. ✅ 2026-09-12 — honoured. The new README documents the platform-qualified arrangement Phase 10 produced (six test databases, Windows and Linux concurrent) rather than the one-database-per-suite text that would have had to be rewritten immediately.
- [x] Note the harness path inconsistency documented back in 1.4: knottyyoga uses `server/docker_project/`, communityfinder uses `server/docker/`. Worth converging while the files are being moved anyway — and CLAUDE.md's generic reference to `server/docker/` is only correct for communityfinder. ✅ 2026-09-12 — **converged rather than merely noted.** `knottyyoga/server/docker_project/` is now `knottyyoga/server/docker/`, matching communityfinder and server_components. Blast radius was measured first and was entirely knottyyoga-local: 9 files, all textual references, with the container's named volumes (`knottyyoga-linux-build`, `honuware-conan2`) fixed strings and every path in `load_container.cmd` resolved relative to `%~dp0`, so nothing was path-derived. CLAUDE.md's generic `server/docker/` reference is now correct for both apps. Verified by the knottyyoga Linux gate running through the renamed `./docker/build_and_test.sh`: **5174 passed**.

  One trap worth recording: `sed -i` over the `.cmd` files silently stripped their CRLF endings, which `.gitattributes` requires (`*.cmd text eol=crlf`). Caught by counting CR bytes per file afterwards and restored. A repo with a per-extension line-ending policy needs that check after any bulk text edit.
...
- [x] **`Machine configuration.md` is a stub with a real setup buried in it.** Sections *System Overview*, *Setup Instructions*, *Directory Structure* and *Test Overview* are all unfilled template prompts, while the genuinely load-bearing content is the eleven lines at the top. Fold those into the new README and let the vault document either point at it or be retired. ✅ 2026-09-12 — folded into the README and the vault document rewritten as a pointer. It keeps a table of what each of the eleven lines became, so the trail from the old instruction to the corrected one survives; the four unfilled template sections are gone.
- [x] Typo: "Install Pytyhon". ✅ 2026-09-12
- [x] **Explain *why* Python is needed**, or it will be dropped as unnecessary by the next person: libpq 17.11 builds with Meson, which is a Python application (6.2b). The instruction also needs the Store-alias warning beside it. ✅ 2026-09-12 — in the README's prerequisites and again in the vault pointer: `libpq` builds with Meson, which is a Python application, so a broken Python breaks a C++ dependency with an error that never mentions Python. The Store-alias warning sits beside it in both places.
- [x] **"Delete `CMakeUserPresets.json` before opening in VS2026" deserves its cause.** It is regenerated by `conan install --output-folder` pointing outside the repo (the Phase 4 housekeeping note), and it is gitignored. Stating the cause turns a superstition into a diagnosis. ✅ 2026-09-12 — the instruction is gone, and the vault document now says *why* it is gone: the cause was fixed at source in 8.2, so keeping the step would have preserved a superstition after its reason was removed. Separately, `knottyyoga/CLAUDE.md` still told you to put `FETCHCONTENT_SOURCE_DIR_HONUWARE` in `CMakeSettings.json`, which 8.1 deleted — now corrected to `CMakePresets.json`/`CMakeUserPresets.json`.
- [x] **Pin versions in the setup doc or stop naming them.** It names CMake 4.4.3 and Conan 2.31.2 but links to download landing pages that serve whatever is current — so the document will drift silently. Either link the exact release or say "at least X". ✅ 2026-09-12 — the README's prerequisites table names the exact pins and carries a standing note that a version changed in the tree must be changed there too, on the stated grounds that a named version beside a "whatever is current" download link drifts silently and is worse than naming nothing.
- [x] `knottyyoga/database_server/Dockerfile.archive` and `Dockerfile.old` are `FROM gcc:7.3.0` and appear dead. Confirm and delete rather than carrying them into the moved layout. ✅ 2026-09-12 — confirmed dead and deleted. Both were `FROM gcc:7.3.0` attempts to build PostgreSQL from scratch; `Dockerfile.old`'s own header said *"This file isn't used anymore"*. `clear-database.cmd` went with them — `CLAUDE.md` already called it dead, and what it actually did was delete the cluster directory, which is not "a decent starting state" as `SERVER.md` claimed.
- [x] Revisit `knottyyoga/server/docker_project/conan_profiles/default` — Phase 2 found it inert (nothing references it) and drifted (`compiler.cppstd=gnu20` while the build passes `17`). Delete it or wire it up deliberately; moving it as-is would propagate a known-dead file. ✅ 2026-09-12 — deleted. Re-confirmed unreferenced (grep across all three repos, CI included, returns nothing) and still drifted at `compiler.cppstd=gnu20` against a build that passes 17. Moving it into the new layout would have propagated a known-dead file.
...
- [x] **Feed this into Phase 14's README as a first-class setup step.** "Create and seed the dev database" is currently written down nowhere, which is precisely why it was missed — `Machine configuration.md` stops at cloning the repo. A new developer will hit this exact error. ✅ 2026-09-12 — done as step 7 (*Create and seed the dev database*), with step 8 (*Know which database you are looking at*) beside it so the failure mode is named as well as the fix.
